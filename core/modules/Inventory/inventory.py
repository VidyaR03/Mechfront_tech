...
def inventory_overview(request, id):
    ...
    if download_csv:
        ...
        response['Content-Disposition'] = f'attachment; filename=inventory_{inventory_entity_data.item_code}.csv'
    ...
    # --- Render Template if not downloading ---
...
def inventory_data_add(request):
    ...
    try:
        if request.method == "POST":
            ...
            'unit_per_case':request.POST['unit_per_case'],
            'date': current_date,
...
def inventory_edit(request, id):
    ...
    if request.method == 'POST':
        try:
            ...
            inventory_item.unit_per_case = request.POST['unit_per_case']
